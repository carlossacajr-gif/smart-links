...
export default function LoginForm() {
    ...
    return (
        <div className="w-full max-w-[420px] mx-auto pt-10">
            ...
            >
                <div className="absolute -top-1/2 -right-1/2 w-full h-full bg-[#EB3333]/15 blur-[100px] rounded-full pointer-events-none transition-all duration-1000 group-hover:bg-[#EB3333]/20 group-hover:scale-110 mix-blend-multiply"></div>

                <div className="relative z-10">
                    <motion.div
                        initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3, duration: 0.8 }}
                        className="w-16 h-16 rounded-3xl bg-white/50 border border-black/5 flex justify-center items-center backdrop-blur-md mb-8 relative overflow-hidden shadow-sm"
                    >
                        <div className="absolute inset-[-2px] rounded-[34px] border border-[#EB3333]/20 animate-[spin_3s_linear_infinite]"></div>

                        {/* Logo Inteligente (Claro en Oscuro, Rojo en Claro) */}
                        <img src="/logos/Logo rojo (solo icono).svg" alt="Saca Logo" className="w-8 h-8 relative z-10 block opacity-90 transition-transform duration-500 scale-105" />
                    </motion.div>

                    <motion.h1
                        initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.8 }}
                        className="text-4xl font-display font-bold text-zinc-900 tracking-tight leading-tight mb-2 transition-colors"
                    >
                        Acceso seguro
                    </motion.h1>
                    <motion.p
                        initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5, duration: 0.8 }}
                        className="text-zinc-500 text-[15px] font-medium mb-10 transition-colors"
                    >
                    ...
                    <form onSubmit={handleAuth} className="space-y-7">
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.8 }}>
                            ...
                            <input
                                ...
                                className="w-full bg-white/50 border border-zinc-200 py-[1.125rem] px-6 rounded-[1.25rem] text-[15px] text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-4 focus:ring-[#EB3333]/20 focus:border-[#EB3333]/50 transition-all font-medium backdrop-blur-sm"
